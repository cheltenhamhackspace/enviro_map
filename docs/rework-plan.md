...
| WP4 | **Hourly rollups** | Migration + batched backfill + second ingest upsert + analysis endpoints (statistics/compare/trends/spatial) move to rollups + `bucket=hour` on the readings GET. Includes the rollup-vs-raw verification mode. | **High — backfill + ingest + result-parity checks. Do not combine with anything else.** |
